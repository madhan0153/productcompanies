...
export function TailorReview({
  ...
  const [finalisedUrls, setFinalisedUrls] = useState<{ pdf: string; print: string } | null>(null);
  void rowId;
  ...
  const onFinalise = () => {
    ...
    startFinalising(async () => {
      try {
        ...
        if (res.ok) {
          setFinalisedUrls({ pdf: res.pdf_url, print: res.print_url });
          router.refresh();
        } else {
    ...
    <div className="space-y-4 pb-32">
      ...
      {diagnosis.weak_bullets.length === 0 ? (
        <div className="rounded-xl border border-success/30 bg-success/5 p-4 text-center text-sm text-success">
          ...
          Your resume already covers this JD well. Finalise to render the application-ready PDF.
        </div>
      ) : (
        <section>
      ...
      {finalisedUrls && (
        <div className="rounded-xl border border-success/30 bg-success/5 p-4">
          ...
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
            Ready for download. Use the PDF for applications.
          </p>
