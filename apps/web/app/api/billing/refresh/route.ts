...
// Wrapped: refreshEntitlements does a DB upsert, so a transient DB error
// returns a logged, safe 500 instead of an opaque framework crash.
export const POST = safeRoute("billing.refresh", async (req: NextRequest) => {
  ...
  if (!user) {
    return unauthorized("Please sign in to refresh your plan.");
  }
